...
const Details = ({position,company,companyLink,time,address,work})=>{
    ...
    return <li ref={ref} className='my-8 first:mt-0 last:mb-0 w-[60%] mx-auto flex flex-col items-center justify-between md:w-[80%]'>
        ...
        <motion.div initial={{y:50}} whileInView={{y:0}} transition={{duration:0.5,type:"spring"}}>
            <h3 className='capitalize font-bold text-2xl sm:text-xl xs:text-lg text-light'>{position}&nbsp; <a target='_blank' className=' text-primaryDark capitalize' href={companyLink}>@{company}</a></h3>
            <span className='capitalize font-medium text-light/75 xs:text-sm'>
                {time} | {address}
            </span>
            <p className='font-medium w-full text-light md:text-sm'>
                {work}
            </p>
        </motion.div>
    </li>
  ...
  return (
    ...
    <div className='mt-10 mb-32'>
        ...
        <div ref={ref} className='w-[75%] mx-auto relative lg:w-[90%] md:w-full'>
            <motion.div style={{scaleY:scrollYProgress}} className='absolute left-9 top-0 w-[4px] h-full origin-top bg-light md:w-[2px] md:left-[30px] xs:left-[20px]'/>
